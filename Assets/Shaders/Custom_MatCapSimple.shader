  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          ...
          out_v.xlv_TEXCOORD0 = ((mul(tmpvar_5, normalize((((v_2.xyz * in_v.normal.x) + (v_3.xyz * in_v.normal.y)) + (v_4.xyz * in_v.normal.z)))).xy * 0.5) + 0.5);
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          OUT_Data_Frag out_f;
          float4 tmpvar_1;
          tmpvar_1 = tex2D(_MainTex, in_f.xlv_TEXCOORD1);
          float4 tmpvar_2;
          tmpvar_2 = tex2D(_MatCap, in_f.xlv_TEXCOORD0);
          float4 tmpvar_3;
          tmpvar_3 = ((_Color * tmpvar_1) * ((tmpvar_2 * _MatCapStrength) * 2));
